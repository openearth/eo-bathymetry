...
# Area of interest
aoi_file_path = (
    r"p:\11209821-cmems-global-sdb\00_miscellaneous\AOIs\AOI_GER_WaddenSea.geojson"
)
...
# Select tile
tile = ee.Feature(
    tiles.filterMetadata("tx", "equals", "1065.0")
    .filterMetadata("ty", "equals", "660.0")
    .first()
)
...
def add_gtsm_gebco_data_to_images(
    ...
    tile = ee.Feature(
        ee.Algorithms.If(
            ee.Algorithms.IsEqual(tile.geometry(), None),
    ...
    image_period = ee.DateRange(
        ee.Date(image_time_start.millis().subtract(max_temporal_offset * 60 * 1000)),
    ...
    # Get gtsm feature
    gtsm_feature = ee.Feature(
        ee.Algorithms.If(bool_empty, ee.Feature(None), gtsm_col.first())
    )
    ...
    station_buffer = ee.Geometry(
        ee.Algorithms.If(
            ...
            gtsm_feature.geometry(),
        )
    ).buffer(max_spatial_offset * 1000)
    ...
    gtsm_tidal_stage_percentage = ee.Number(
        ee.Algorithms.If(
            bool_empty, None, get_gtsm_tidal_stage_percentage(gtsm_feature, gebco_data)
        )
    )
...
print(
    "{} s to add gtsm and gebco data to the first image".format(
...
t0 = time.time()
image_col2 = image_col.map(
    lambda image: add_gtsm_gebco_data_to_images(image, gtsm_col, gebco_image, tile)
)
print(
    "{} s to add gtsm and gebco data to all images".format(
        np.round(time.time() - t0, 2)
    )
)
...
def get_tide_offsets_and_spread(image_col):
    ...
    high_tide_offset = ee.Number(100).subtract(
        ee.Number(
            image_col_.reduceColumns(
    ...
    low_tide_offset = ee.Number(
        image_col_.reduceColumns(ee.Reducer.min(), ["gtsm_tidal_stage_percentage"]).get(
            "min"
        )
    )

    # Get tide spread
    tide_spread = ee.Number(100).subtract(high_tide_offset).subtract(low_tide_offset)
...
if plot_image:
    ...
    if not image_properties["gtsm_gebco_data_isempty"]:
        gtsm_feature = ee.Geometry.Point(
            [image_properties["gtsm_station_lon"], image_properties["gtsm_station_lat"]]
        )
...
def get_gtsm_gebco_data_from_image(image):
    ...
    feature = ee.Feature(
        ee.Algorithms.If(
            image.get("gtsm_gebco_data_isempty"),
            feature.set(
                {
                    "gtsm_station": None,
            ...
            feature.set(
                {
                    "gtsm_station": image.get("gtsm_station"),
                    ...
                    "gtsm_tidal_stage_percentage": image.get(
...
gtsm_gebco_data = image_col2.map(
    lambda image: get_gtsm_gebco_data_from_image(image)
).getInfo()
image_idxs = np.arange(0, len(gtsm_gebco_data["features"]))
gtsm_waterlevels = np.array(
    [
        feature["properties"]["gtsm_waterlevel"]
        for feature in gtsm_gebco_data["features"]
    ]
)
gebco_hats = np.array(
    [feature["properties"]["gebco_hat"] for feature in gtsm_gebco_data["features"]]
)
gebco_lats = np.array(
    [feature["properties"]["gebco_lat"] for feature in gtsm_gebco_data["features"]]
)
gtsm_tidal_stage_percentages = np.array(
    [
        feature["properties"]["gtsm_tidal_stage_percentage"]
        for feature in gtsm_gebco_data["features"]
    ]
)
...
axs[0].add_patch(
    plt.Rectangle(
        (0, np.min(gtsm_waterlevels)),
        len(image_idxs),
        np.max(gtsm_waterlevels) - np.min(gtsm_waterlevels),
        color="blue",
        alpha=0.1,
    )
)
axs[0].scatter(
    image_idxs, gtsm_waterlevels, color="blue", s=3, label="Gtsm water level"
)
axs[0].plot(
    image_idxs,
    gebco_lats,
    color="green",
    linewidth=3,
    label="Gebco lowest astronomical tide",
)
axs[0].plot(
    image_idxs,
    gebco_hats,
    color="red",
    linewidth=3,
    label="Gebco highest astronomical tide",
)
...
axs[1].add_patch(
    plt.Rectangle(
        (0, np.min(gtsm_tidal_stage_percentages)),
        len(image_idxs),
        np.max(gtsm_tidal_stage_percentages) - np.min(gtsm_tidal_stage_percentages),
        color="blue",
        alpha=0.1,
    )
)
axs[1].scatter(
    ...
    color="blue",
    s=3,
    label="Gtsm tidal stage percentage",
)
